...
import { diffProjects, formatDiff } from "../lib/diff.mjs";
...
使い方:
  ...
  pbip-viewer dax <path> "<式>" [--table <T>] [--json]
      埋め込みデータに対しDAX式を評価して結果を出力。
  pbip-viewer diff <pathA> <pathB> [--json]
      2つのPBIPのページ/ビジュアル/メジャー/列/リレーション/検査の差分。
  pbip-viewer report <path> [-o <out.md>]
      Markdownレポート(概要・メジャーDAX・検出事項・リレーション)を出力。

例:
  pbip-viewer analyze ./MyReport --json --pretty
  pbip-viewer check ./MyReport && echo OK
  pbip-viewer dax ./MyReport "SUM(売上[金額])"
  pbip-viewer diff ./before ./after
  pbip-viewer report ./MyReport -o report.md
`;
...
const ALLOWED_FLAGS = {
  ...
  dax: new Set(["table", "json", "help"]),
  diff: new Set(["json", "help"]),
  report: new Set(["out", "help"]),
};
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (a === "-h" || a === "--help" || a === "help") {
      ...
      const next = argv[i + 1];
      if (next === undefined || next.startsWith("-")) args.errors.push("-t/--table にはテーブル名が必要です。");
...
async function main() {
  ...
  if (cmd === "dax") {
    const expression = _[1];
    if (!expression) {
      process.stderr.write('DAX式を指定してください。例: pbip-viewer dax ./MyReport "SUM(売上[金額])"\n');
      process.exitCode = 2;
      return;
    }
    const { project } = await loadProject(inputPath);
    const result = await evaluateExpression(project, expression, flags.table);
    if (flags.json) process.stdout.write(JSON.stringify({ expression, table: flags.table || null, result }) + "\n");
    else process.stdout.write(`${result instanceof Date ? result.toISOString().slice(0, 10) : result}\n`);
    return;
  }
  ...
  if (cmd === "diff") {
    const pathB = _[1];
    if (!pathB) {
      process.stderr.write("比較対象を2つ指定してください: pbip-viewer diff <A> <B>\n");
      process.exitCode = 2;
      return;
    }
    const { project: a } = await loadProject(inputPath);
    ...
    else process.stdout.write(formatDiff(diff, projectBaseName(inputPath), projectBaseName(pathB)) + "\n");
    return;
  }
  ...
  if (cmd === "report") {
    ...
    if (flags.out) {
      await fsp.writeFile(path.resolve(flags.out), md);
      process.stdout.write(`書き出しました: ${path.resolve(flags.out)} (${md.length} bytes)\n`);
    } else {
