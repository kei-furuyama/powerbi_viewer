...
async function walk(dir, issues, visited) {
  const out = [];
  let dirents;
  try {
  ...
  for (const entry of dirents) {
    ...
    if (entry.isDirectory()) {
      out.push(...(await walk(full, issues, visited)));
      ...
      if (st?.isDirectory()) out.push(...(await walk(real, issues, visited)));
...
export async function readEntriesFromDir(dir) {
  ...
  const issues = [];
  const files = await walk(root, issues, new Set([root]));
  const entries = [];
