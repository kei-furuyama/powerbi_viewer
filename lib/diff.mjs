...
function pageMap(project) {
  const map = new Map();
  for (const p of project?.report?.pages || []) map.set(p.displayName || p.id, (p.visuals || []).length);
  return map;
}
...
export function diffProjects(a, b) {
  ...
  const pb = pageMap(b);
  const pagesChanged = [];
  for (const [name, countA] of pa) {
    if (pb.has(name) && pb.get(name) !== countA) pagesChanged.push({ page: name, fromVisuals: countA, toVisuals: pb.get(name) });
  }

  return {
    pages: { added: added(new Set(pa.keys()), new Set(pb.keys())), removed: added(new Set(pb.keys()), new Set(pa.keys())), changedVisualCount: pagesChanged },
